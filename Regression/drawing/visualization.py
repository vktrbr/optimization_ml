...
def gen_exponential_plot(x: np.ndarray, y: np.ndarray, w: np.ndarray) -> go.Figure:
    """
    y = w[0] * exp(X @ w[2:]) + w[1]
    ...
    min_max_delta = 0.01 * (np.max(x) - np.min(x))
    x_axis = np.linspace(np.min(x) - min_max_delta, np.max(x) + min_max_delta, 50)
    y_axis = w[0] * np.exp(x_axis.reshape(-1, 1) @ w[2:].reshape(-1, 1)) + w[1]
    fig = px.scatter({'x': x.reshape(-1), 'initial distr': y}, x='x', y=['initial distr'],
                     title='<b>Regression plot</b>')
